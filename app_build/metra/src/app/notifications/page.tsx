...
export default function NotificationsPage() {
  ...
  const fetchInbox = async () => {
    try {
      setLoading(true);
      const res = await fetch("http://localhost:8000/api/v1/notifications/inbox?limit=100");
      if (res.ok) {
  ...
  const handleSendTest = async (template: string) => {
    try {
      ...
      const res = await fetch("http://localhost:8000/api/v1/notifications/send-test", {
        method: "POST",
  ...
  const handleClearInbox = async () => {
    ...
    try {
      setLoading(true);
      const res = await fetch("http://localhost:8000/api/v1/notifications/clear", {
        method: "POST",
      });
      if (res.ok) {
        setNotifications([]);
